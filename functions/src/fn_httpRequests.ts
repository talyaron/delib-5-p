...
export async function hashPassword(req: any, res: any) {
  try {
    ...
    const saltRounds = 10;
    const hashedPassword = await bcrypt.hash(plainPassword.toString(), saltRounds);

    if (hashedPassword) {
      const docRef = await addDoc(collection(db, Collections.statementsPasswords), {
        statementId: statementId,
        hashedPassword: hashedPassword,
      });
      console.log("Hashed password stored with ID:", docRef.id);
      return res.status(200).json({ ok: true, docId: docRef.id });
    } else {
      return res.status(500).json({ ok: false, error: "Password could not be hashed" });
    }
  } catch (error: any) {
...
export async function checkPassword(req: any, res: any) {
  try {
    ...
    if (match) {
      ...
      return res.send("Password and Hashed Password don't match");
    }
  } catch (error: any) {
    res.status(500).send({ error: error.message, ok: false });
    return;
