...
export async function countRoomJoiners(ev: any) {
    try {
        ...
        if (previouseRequest === undefined) {
            //get the room
            const lobbyRoomRef = db.collection(Collections.statementLobbyRooms).doc(newRequest.statementId);
            ...
            const previousLoobyRoomRef = db.collection(Collections.statementLobbyRooms).doc(previouseRequest.statementId);
            ...
            const newLobbyRoomRef = db.collection(Collections.statementLobbyRooms).doc(newRequest.statementId);
            const previousLoobyRoomRef = db.collection(Collections.statementLobbyRooms).doc(previouseRequest.statementId);
            //update the room with the new count
            ...
            await updateNewRoom(newLobbyRoom, newLobbyRoomRef, newRequest.parentId, newRequest.statementId);

            await updatePreviuosRoom(previousLoobyRoom, previousLoobyRoomRef, previouseRequest.parentId, previouseRequest.statementId);
        };
    } catch (error) {
        logger.error(error);
    }
