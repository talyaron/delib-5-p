...
export function getSectionObj(
	...
	try {
		...
		const { title, paragraphs, sectionsString } = getLevelTexts(
			text,
			nextLevel
		);
		...
		if (sectionsString.length > 0) {
			sections = sectionsString.map(
				(sct) => getSectionObj(sct, nextLevel) as Sections
			);
		}
...
export function getParagraphs(text: string, level: number): string[] {
	try {
		...
		texts.splice(0, 1);
		const nextSectionIndex = texts.findIndex((text) =>
