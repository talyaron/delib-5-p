...
const QuestionDashboard: FC<Props> = ({ statement }) => {
	try {
		...
		const metaData: StatementMetaData | undefined = useAppSelector(
			statementMetaDataSelector(statement.statementId)
		);
		...
		const questionType =
			statement.questionSettings?.questionType || QuestionType.singleStep;
