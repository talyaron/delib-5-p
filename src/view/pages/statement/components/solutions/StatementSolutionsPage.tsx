...
const StatementEvaluationPage: FC<StatementEvaluationPageProps> = ({
	...
	try {
		...
		const renderCommonContent = () => (
			<>
				<div className={styles.addingStatementWrapper} style={{ paddingTop: "2rem" }}>
					<div className={styles.header}>
						<div className={styles.title}>
							<h1 className={styles.h1}>
								{smallScreen ? (
									<>
										{t(`Click on`)} <span className={styles.titleSpan}>{t(`”+”`)}</span> {t(`to add your ${currentPage}`)}
									</>
								) : (
									<>
						...
						>
							{smallScreen ? (
								<WhitePlusIcon />) : ( <p> {t(`Add ${currentPage}`)} <WhitePlusIcon /> </p>
							)}
						</div>
					</div>
