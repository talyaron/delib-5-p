...
export async function getStatmentsSubsciptions(): Promise<StatementSubscription[]> {
  try {
    ...
    if (!user.uid) throw new Error("User not logged in");
    const statementsSubscribeRef = collection(DB, Collections.statementsSubscribe);
    const q = query(statementsSubscribeRef, where("userId", "==", user.uid), limit(40));
...
export async function getStatementSubscriptionFromDB(
  ...
  try {
    ...
    if (!statementSubscrionId) throw new Error("Statement subscription id is undefined");

    const subscriptionRef = doc(DB, Collections.statementsSubscribe, statementSubscrionId);
    const subscriptionDB = await getDoc(subscriptionRef);
...
export async function getTopParentSubscription(statementId: string): Promise<GetTopParentSubscriptionProps> {
  try {
